...
pub enum Attractor<T> {
    ...
    Soph { a: T, b: T, c: T, d: T },
    CliffordSoph { a: T, b: T, c: T, d: T },
}
...
impl<T: Add<Output = T> + Copy> Attractor<T> {
    pub fn shift(&mut self, delta: T) {
        match self {
            ...
            Attractor::CliffordSoph { a, b, c, d } => {
                *self = Attractor::CliffordSoph {
                    a: *a + delta,
                    b: *b + delta,
                    c: *c + delta,
                    d: *d + delta,
    ...
    T: Copy + Add<Output = T> + Sub<Output = T> + Mul<Output = T> + PartialOrd + Float + NumCast,
{
    ...
    pub fn iterate(&self, p: Complex<T>) -> Complex<T> {
        match self {
            ...
            Attractor::Soph { a, b, c, d } => soph(p, *a, *b, *c, *d),
            Attractor::CliffordSoph { a, b, c, d } => soph(clifford(p, *a, *b, *c, *d), *a, *b, *c, *d),
        }
    }
}
...
#[inline(always)]
fn soph<T>(p: Complex<T>, a: T, b: T, c: T, d: T) -> Complex<T>
where
    T: Copy + Add<Output = T> + Mul<Output = T> + Float + NumCast,
{
    let x = p.real.powi(2);
